...
class ToolConfig {
  ...
  /// `value` (ordinary knob) or `model` (a provider_id/model_id reference the
  /// UI renders as a picker scoped to [capability]).
  final String kind;
...
/// types only). Used until ListProvidersCatalog answers (or offline); the
/// server response overrides this whenever it loads.
const Map<String, List<String>> kFallbackApiTypeCapabilities = {
  'openai-compatible': [
...
class ProviderInfo {
  ...
  /// The single modality this provider serves (semantic grouping): all of its
  /// models share it. text | image | video | speech | transcription |
  /// embedding | rerank | realtime.
  final String capability;
...
class ProviderDraft {
  ...
  /// The single modality this provider serves (set by the section the user
  /// added it from; fixed while editing).
  String capability;
  String apiType;
  String baseUrl;
  String apiKey;
