    ...
    return MarkdownBody(
      ...
      styleSheet: MarkdownStyleSheet(
        p: muted
            ...
            : text.body,
        h1: text.body.copyWith(fontSize: 18, fontWeight: FontWeight.w600),
        h2: text.body.copyWith(fontSize: 16, fontWeight: FontWeight.w600),
        ...
        tableBody: text.meta,
        // The bundled family has no Bold face: pin markdown emphasis to the
        // SemiBold(600) instance the other clients use for bold text.
        strong: text.body.copyWith(fontWeight: FontWeight.w600),
...
class MessageBubble extends StatelessWidget {
  ...
  /// Retry withdraws the message (and everything after) then resends — a
  /// destructive action, so confirm first (same policy as undo).
  Future<void> _retry(BuildContext context) async {
    final ok = await showDialog<bool>(
      context: context,
      builder: (ctx) => AlertDialog(
        title: Text(ctx.l10n.retryTitle),
        content: Text(ctx.l10n.retryBody),
        actions: [
          TextButton(
  ...
  Widget build(BuildContext context) {
    ...
    return Align(
      ...
      child: Row(
        ...
        children: [
          if (showCopy) ...[
            _tinyIcon(AppIcons.copy, context.l10n.copy, onCopy, colors),
            const SizedBox(width: 2),
          ],
          if (showResend) ...[
            _tinyIcon(
                AppIcons.refresh, context.l10n.retry, onResend, colors),
            const SizedBox(width: 2),
          ],
          if (showEdit) ...[
            _tinyIcon(AppIcons.edit, context.l10n.edit, onEdit, colors),
            const SizedBox(width: 2),
          ],
          _tinyIcon(AppIcons.undo, context.l10n.undo, onUndo, colors),
