...
/// The (bg, fg, hue) a seed renders — the exact contract the four clients
/// share (see tools/avatars.py). Used by the widget and by the parity test.
( Color, Color, int ) debugAvatarSpec(String seed) {
  final hue = ChatAvatar._hue(seed);
  final bg = HSLColor.fromAHSL(1, hue, 0.60, 0.48).toColor();
  const darkRungs = [0.34, 0.28, 0.22, 0.17, 0.12];
  const lightRungs = [0.66, 0.72, 0.78, 0.84, 0.90];
  Color best = Colors.white;
  var bestRatio = -1.0;
  for (final l in [...darkRungs, ...lightRungs]) {
    final c = HSLColor.fromAHSL(1, hue, 0.62, l).toColor();
    final r = ChatAvatar._contrastRatio(c, bg);
    if (r > bestRatio) {
      bestRatio = r;
      best = c;
    }
  ...
  final fg = bestRatio < 3.0
      ? (ChatAvatar._luminance(bg) > 0.35 ? const Color(0xFF17181C) : Colors.white)
      : best;
  return (bg, fg, hue.round());
}
