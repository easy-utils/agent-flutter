...
class LocalStore {
  ...
  /// Open the on-disk / IndexedDB database for [scope] (the active
  /// gateway+token identity). A different scope yields its own file, so
  /// switching users never shows another tenant's cache. When the scope
  /// changes the previous database is CLOSED and replaced, otherwise the two
  /// connections would race on the same in-memory state.
  static Future<LocalStore> open(String scope) async {
    if (_instance != null && _instance!.scope == scope) return _instance!;
    final old = _instance;
    _instance = LocalStore._(LocalDb(scope));
    if (old != null) {
      // Best-effort: a failed close must not block the new connection.
      try {
        await old._db.close();
      } catch (_) {}
    }
  ...
  /// The connection scope this store belongs to.
  String get scope => _db.scope;
