...
class MessagesController extends ChangeNotifier {
  ...
  /// keeping the CURRENT list order on ties (stable). Ties must NOT be broken
  /// by id: a locally-appended user bubble and the assistant streaming
  /// placeholder created right after it share a millisecond, and `m…` sorts
  /// before `u…` — which would draw the assistant reply ABOVE its user message.
  void _renumber() {
    final indexed = [for (var i = 0; i < messages.length; i++) (i, messages[i])];
    indexed.sort((a, b) {
      final at = DateTime.tryParse(a.$2.createdAt)?.millisecondsSinceEpoch ?? 0;
      final bt = DateTime.tryParse(b.$2.createdAt)?.millisecondsSinceEpoch ?? 0;
      if (at != bt) return at - bt;
      return a.$1 - b.$1;
    });
    messages = [
      for (var i = 0; i < indexed.length; i++) indexed[i].$2.copyWith(seq: i),
    ];
    ...
    for (final cb in _sessionListeners) {
      ...
      case 'tool-input-start':
        ...
        if (event == 'text-start' && params['id'] != null) {
          ...
          _startToolPart(sid, params['id'] as String,
      ...
      case 'tool-input-delta':
        if (params['id'] != null && params['delta'] != null) {
          _appendToolInput(
              params['id'] as String, params['delta'] as String? ?? '');
        }
        break;
      case 'text-delta':
  ...
  void _appendToolInput(String partId, String delta) {
    ...
    final parts = messages[idx].parts.map((p) {
      ...
      return p.copyWith(
          state: old.copyWith(inputText: (old.inputText ?? '') + delta));
    }).toList();
    final next = [...messages];
    next[idx] = messages[idx].copyWith(parts: parts);
    messages = next;
