...
class _ConfigScreenState extends State<ConfigScreen> {
  ...
  Widget _listView(BuildContext context) {
    return ListView(
      children: [
        ...
        _listTile(context, AppIcons.globe, 'language', _pickLanguage),
        _listTile(
          context,
          AppIcons.language,
          'agentLocale',
  ...
  Future<void> _pickLanguage() async {
    final picked = await showDialog<String>(
      ...
      builder: (ctx) => SimpleDialog(
        ...
        children: [
          for (final (code, label) in [
            ('system', ctx.l10n.followSystem),
            ('zh', '中文'),
            ('en', 'English'),
          ])
            ListTile(
              leading: Icon(
                I18n.pref == code ? AppIcons.target : AppIcons.circle,
                color: colorsOf(ctx).primary,
              ),
              title: Text(label),
              onTap: () => Navigator.pop(ctx, code),
  ...
  Widget _appearance(BuildContext context) {
    ...
    final label = switch (widget.themeMode) {
  ...
  Future<void> _pickTheme() async {
    final picked = await showDialog<String>(
      context: context,
      builder: (ctx) => SimpleDialog(
        title: Text(ctx.l10n.appearance),
        children: [
          for (final (code, label) in [
            ('system', ctx.l10n.followSystem),
            ('light', ctx.l10n.themeLight),
            ('dark', ctx.l10n.themeDark),
          ])
            ListTile(
              leading: Icon(
                widget.themeMode == code ? AppIcons.target : AppIcons.circle,
                color: colorsOf(ctx).primary,
              ),
              title: Text(label),
              onTap: () => Navigator.pop(ctx, code),
    ...
    if (picked == null || picked == widget.themeMode) return;
    widget.onThemeMode(picked);
    setState(() {});
  }
...
class _ToolsDetailState extends State<_ToolsDetail> {
  ...
  Widget _toolCard(ToolInfo tool) {
    final colors = colorsOf(context);
    final text = textOf(context);
    // A value that was explicitly unset is stored as '' — treat it as unset.
    final hasConfig = (_config[tool.name] ?? const <String, dynamic>{})
        .values
  ...
  Widget _extConfigEditor(ToolInfo tool, ToolConfig c) {
    ...
    // The extension DECLARES model-reference knobs (kind == 'model' +
    // capability), so the picker lists exactly the models registered under
    // that modality — no variable-name heuristics.
    if (c.isModelRef) {
    ...
    // Selection knobs (enum / boolean) save IMMEDIATELY on pick, like the
    // model picker and the webui <select>; only free text gets a Save button.
    if (c.type == 'enum' && c.enumValues.isNotEmpty) {
      return _GenerativeModelPicker(
        ...
        options: [
          ('', context.l10n.none),
          for (final ev in c.enumValues) (ev, ev),
        ],
        initialValue: current == null ? '' : '$current',
        onSave: (v) => _saveExtConfig(extId, c.name, v),
      );
  ...
  Future<void> _saveExtConfig(String extId, String name, Object? value) async {
    final raw = value == null ? '' : '$value';
    try {
      // Empty means UNSET: write the declared zero value so the config store
      // clears the knob (there is no delete RPC) — the picker then shows 无.
      await widget.api.setToolConfigValue(extId, name, raw);
      setState(() {
        ...
        for (final t in _tools) {
          ...
          final next = <String, dynamic>{...cfgMap};
          if (raw.isEmpty) {
            next.remove(name);
          } else {
            next[name] = value;
...
class _GenerativeModelPicker extends StatefulWidget {
  ...
  /// The modality the declared model reference must match: the picker lists
  /// only providers of this capability (and their models).
  final String capability;
  final Map<String, ProviderInfo> providers;

  /// Explicit options for non-model knobs (enum / boolean). Empty = derive from
...
class _GenerativeModelPickerState extends State<_GenerativeModelPicker> {
  ...
  Widget build(BuildContext context) {
    final text = textOf(context);
    final colors = colorsOf(context);
    // Either explicit options (enum/boolean) or the models of providers that
    // serve the declared modality, flattened to `provider_id/model_id` refs.
    final refs = <(String, String)>[];
    if (widget.options.isNotEmpty) {
      refs.addAll(widget.options);
    } else {
      for (final p in widget.providers.values) {
        if (p.capability != widget.capability) continue;
      ...
      child: Column(
        ...
        children: [
          ...
          DropdownButtonFormField<String>(
            // Long refs (`provider_id/model_id`) must wrap instead of being
            // clipped, so both the closed field and the menu items wrap.
            isExpanded: true,
            ...
            items: [
              // Explicit options (enum/boolean) already carry their own "无"
              // entry; the model list prepends one so the knob can be unset.
              ...
              for (final (ref, name) in refs)
                DropdownMenuItem(
                  value: ref,
                  child: Text(
                    name == ref ? ref : '$name —— $ref',
                    softWrap: true,
                  ),
                ),
            ],
            // Pick → save IMMEDIATELY (no Save button), matching webui.
            onChanged: (v) => setState(() {
              _selected = v ?? '';
              widget.onSave(_selected);
            }),
          ),
          if (refs.isEmpty && widget.options.isEmpty)
            Padding(
        ...
        children: [
          ...
          Row(
            ...
            children: [
              ...
              // Always enabled (an empty value is a valid "unset").
              FilledButton.tonal(
